...
pub async fn update_client(adapter: &FineractAdapter, req: UpdateClientReq) -> Result<CallToolResult, McpError> {
    ...
    let mut payload = json!({
        "firstname": current.get("firstname").and_then(|v| v.as_str()).unwrap_or_default(),
        "lastname": current.get("lastname").and_then(|v| v.as_str()).unwrap_or_default(),
    ...
    if let Some(v) = req.firstname { payload["firstname"] = json!(v); }
    if let Some(v) = req.lastname { payload["lastname"] = json!(v); }
